...
export class MediaRoomsController {
  ...
  async createOrGetRoom(@Param('roomId') roomId: string): Promise<MediaRoomResponse> {
  ...
  async createTransport(
    ...
    const transport = await this.mediaRoomsService.createTransport(roomId, peerId, direction);
  ...
  async connectTransport(
    ...
    await this.mediaRoomsService.connectTransport(roomId, peerId, transportId, dtlsParameters);
  ...
  @HttpCode(200)
  removePeer(@Param('roomId') roomId: string, @Param('peerId') peerId: string): RemovePeerResponse {
